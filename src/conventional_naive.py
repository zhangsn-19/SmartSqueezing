...
from constant import PEAK_TROUGH, CONF_BETA, TIME_THRESHOLD, CONFIDENCE
...
import time as tm
from util import getNumofCommonSubstr
...
def analyzeGroups(touchGroups: List[movementGroup] = None) -> bool:
    ...
    for idx, touchGroup in enumerate(touchGroups):
        ...
        if(fakeCompare( myTouch11[idx], Touches ) < CONFIDENCE ):
            result = True
...
def fakeCompare(touch_1: List[touch], touch_2: List[touch]) -> int:
    ...
    """
    try:
        substr, length = getNumofCommonSubstr(touches_1, touches_2)
...
def updateTrack(data: List[List[int]], currentGroups: List[spotGroup]):
...
def test_ground_truth():
    ...
    for control_index in range(0, 10):
        ...
        updateTrack(getFuse(getPressDownDouble()), spot)   
        tm.sleep(0.1)
    for control_index in range(0, 8):
        spot = []
        updateTrack(getFuse(getPressMiddleSingle()), spot)
        tm.sleep(0.2)
    for control_index in range(0, 10):
        spot = []  
        updateTrack(getFuse(getPressDownDouble()), spot)  
...
def test_lll():
    ...
    for control_index in range(0, 10):
        spot = [] # list of spotgroup
        updateTrack(getFuse(getPressUpDouble()), spot)   
        tm.sleep(0.5)
    for control_index in range(0, 8):
        spot = []
        updateTrack(getFuse(getPressMiddleSingle()), spot)
